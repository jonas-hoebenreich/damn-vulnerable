        ...
        public function add_menu()
        {
           if(current_user_can('editor') || current_user_can('administrator')){
                add_menu_page('ContentStudio Publisher', 'ContentStudio', 'edit_posts', 'contentstudio_settings', [
                    $this,
                    'connection_page',
                ], $this->cstu_plugin_assets_dir . "menu-logo.svg", '50.505');
        ...
        public function cstu_create_new_post()
        {
            if (isset($_REQUEST) && isset($_REQUEST['cstu_create_new_post'], $_REQUEST['token'], $_REQUEST['nonce'])) {
                ...
                if ($valid) {
                    ...
                    //validate the user and user capabilities to publish posts
                    $user = get_user_by('ID', $post_author); // get the user by id
                    if (!$user || !$user->has_cap('publish_posts') || !$user->has_cap('edit_posts')) {
                        echo json_encode(['status' => false, 'message' => 'Invalid post author.']);
                        die();
                    }
